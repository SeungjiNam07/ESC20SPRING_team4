...
complete.data = cbind(complete.data, data$Attr70, data$class)

write.csv(complete.data, "new_imputed_data_mice.csv", row.names = FALSE)
```
...
complete_data_knn = cbind(complete_data_knn, data$Attr70, data$class)

write.csv(complete_data_knn, "new_imputed_data_knn.csv", row.names = FALSE)
```
...
for (i in 1:ncol(complete_data_mean)){
  complete_data_mean[,i][is.na(complete_data_mean[,i])] <- mean(complete_data_mean[,i][order(complete_data_mean[,i])[4:(nrow(complete_data_mean)-3)]])
}
...
for (i in 1:ncol(complete_data_med)){
  complete_data_med[,i][is.na(complete_data_med[,i])] <- median(complete_data_med[,i][order(complete_data_med[,i])])
}
